...
(insert virtual filesystem projection image here)
...
In the following cases, 'X' represents a stale replica, '&' represents a good replica, and '-' indicates a replica which does not exist. This table shows the statuses of the two replicas of the source data object for a get. In each case, replica A is being targeted for a get.
...
In the following cases, 'X' represents a stale replica, '&' represents a good replica, and '-' indicates a replica which does not exist. This table shows the statuses of two replicas of the destination data object for a copy. In each case, replica A is being targeted as the destination for a copy and it is assumed that the source data object has an at-rest replica on the requested resource (otherwise, copy would not be *possible*).
...
1. An existing data object with at lesat one replica which is at rest and can be read.
...
The following are all of the columns representing the system metadata in the table as well as their meanings:
 ...
 - `coll_id`: Identifier unique to the collection associated with the data object that maps to this replica. This column will contain the same value for all replicas of a given data object. The identifiers are defined in `R_COLL_MAIN` which contains all of the information about collections in a given zone.
 ...
 - `data_version`: This column is not touched by iRODS anymore. Its purpose is lost to history.
 - `data_type_name`:
 ...
 - `resc_group_name`: This column is not touched by iRODS anymore. It refers to the legacy resource groups feature. This should contain the value `EMPTY_RESC_GROUP_NAME` in order to discourage legacy queries.
 - `resc_name`: This column is not touched by iRODS anymore. This is the name of the storage resource which hosts this replica. This should contain the value `EMPTY_RESC_NAME` in order to discourage legacy queries.
 ...
 - `data_owner_name`: The username of the owner of the data object that maps to this replica. This column will contain the same value for all replicas of a given data object. This value is independent from the Access Control List for the data object which allows for multiple owners to exist. The value of this column will never change, even if the user listed is removed.
 - `data_owner_zone`: The zone of the user that owns the data object that maps to this replica. This column will contain the same value for all replicas of a given data object. This will be populated even if the owner is a user in the local zone.
 ...
 - `data_status`: This column is not touched by iRODS anymore. Its purpose is lost to history.
 ...
 - `data_expiry_ts`: This column is not touched by iRODS anymore. Its purpose is lost to history.
 - `data_map_id`: This column is not touched by iRODS anymore. Its purpose is lost to history.
 - `data_mode`: **Help! The purpose of this column is unknown, but it is still used by iRODS.**
 - `r_comment`: This column is not touched by iRODS anymore. Its purpose is lost to history.
 ...
 - `resc_hier`: This column is not touched by iRODS anymore. This is the semicolon-delimited resource hierarchy which ends with the storage resource which hosts this replica. This should contain the value `EMPTY_RESC_HIER` in order to discourage legacy queries.
